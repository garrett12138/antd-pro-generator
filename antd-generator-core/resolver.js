...
//本文件从依赖 oas-resolver 项目中复制过来，然后注释掉部分代码
//依赖node-fetch-h2导致最后打包的运行不了
        ...
        if (obj[key].startsWith("#")) {
          if (!seen[obj[key]] && !obj.$fixed) {
            ...
            if (target === false) {
              ...
              if (options.fatal) {
                let ex = new Error(
                  "Fragment $ref resolution failed " + obj[key]
                );
            ...
            if (!obj.$fixed) {
              let newRef = (attachPoint + "/" + seen[obj[key]])
                .split("/#/")
                .join("/");
...
/*
//注释掉的函数，应该是处理一些外部引用的url定义，应该用不着
...
function scanExternalRefs(options) {
  return new Promise(function(res, rej) {
        ...
        if (!$ref.startsWith("#")) {
          ...
          if (refs[$ref].resolved) {
            if (options.rewriteRefs) {
              ...
              if (options.verbose > 1)
                console.warn("Rewriting ref", $ref, newRef);
              obj[key]["x-miro"] = $ref;
    ...
    if (
      ...
      options.source === options.resolver.base
    ) {
      ...
      .then(function(refs) {
        for (let ref in refs) {
          if (!refs[ref].resolved) {
            ...
            if (depth > 0) depth++;
            /*
                    //注释掉的代码，应该是处理一些外部引用的url定义，应该用不着
