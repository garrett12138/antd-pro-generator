...
/*
本文件从https://github.com/Mermade/oas-kit/tree/master/packages/swagger2openapi
复制过来，引用该项目时最后打包时会有个依赖有问题导致插件包安装后使用不了，因此复制过来删除了部分代码
  ...
  if (isRef(obj, key)) {
    if (obj[key].startsWith("#/components/")) {
      ...
      obj[key] =
        ...
        common.sanitise(obj[key].replace("#/parameters/", ""));
    } else if (obj[key].startsWith("#/responses/")) {
      // for extensions like Apigee's x-templates
      obj[key] =
        "#/components/responses/" +
        common.sanitise(obj[key].replace("#/responses/", ""));
  ...
  if (scheme.type === "oauth2") {
    ...
    if (typeof scheme.authorizationUrl !== "undefined")
      flow.authorizationUrl =
        scheme.authorizationUrl.split("?")[0].trim() || "/";
...
function fixParamRef(param, options) {
  if (param.$ref.indexOf("#/parameters/") >= 0) {
    ...
    param.$ref =
      refComponents[0] +
      "#/components/parameters/" +
      common.sanitise(refComponents[1]);
  }
...
function processParameter(param, op, path, method, index, openapi, options) {
  ...
  let consumes = ((op ? op.consumes : null) || openapi.consumes || []).filter(
    common.uniqueOnly
  );
  ...
  if (param && param.in === "body") {
    ...
    if (param.schema && param.schema.$ref) {
      ...
      result["x-s2o-name"] =
        decodeURIComponent(
          param.schema.items.$ref.replace("#/components/schemas/", "")
        ) + "Array";
    }
  ...
  if (Object.keys(result).length > 0) {
    ...
    if (op) {
      if (op.requestBody && singularRequestBody) {
        ...
        throwOrWarn(
          "Operation " + opId + " has multiple requestBodies",
          op,
          options
        );
      } else {
        ...
        if (
          ...
          if (
            ...
            delete op.requestBody.content["multipart/form-data"].schema
              .required;
          }
        } else if (
          op.requestBody.content &&
          ...
          if (
            ...
            delete op.requestBody.content["application/x-www-form-urlencoded"]
              .schema.required;
          }
        } else {
...
function processResponse(response, name, op, openapi, options) {
  ...
  if (response.$ref && typeof response.$ref === "string") {
    if (response.$ref.indexOf("#/definitions/") >= 0) {
      ...
      throwOrWarn(
        "definition used as response: " + response.$ref,
        response,
        options
      );
    } else {
      if (response.$ref.startsWith("#/responses/")) {
        response.$ref =
          "#/components/responses/" +
          common.sanitise(
            decodeURIComponent(response.$ref.replace("#/responses/", ""))
          );
      }
    }
  } else {
    ...
    for (let mimetype in response.examples) {
      ...
      response.content[mimetype].examples.response.value =
...
function processPaths(
  ...
  for (let p in container) {
    ...
    if (
      path &&
      path["x-description"] &&
      typeof path["x-description"] === "string"
    ) {
            ...
            for (let e in op["x-ms-examples"]) {
              ...
              if (example.parameters) {
                for (let p in example.parameters) {
                  ...
                  for (let param of (op.parameters || []).concat(
                    path.parameters || []
                  )) {
                    if (param.$ref) {
...
function main(openapi, options) {
  ...
  for (let s in openapi.components.securitySchemes) {
    ...
    if (s != sname) {
      ...
      openapi.components.securitySchemes[sname] =
        openapi.components.securitySchemes[s];
  ...
  for (let s in openapi.components.schemas) {
    ...
    if (s != sname) {
      ...
      openapi.components.schemas[sname + suffix] =
        openapi.components.schemas[s];
  ...
  if (
    ...
    Object.keys(openapi.components.headers).length === 0
  ) {
...
function extractServerParameters(server) {
  ...
  server.url.replace(/\{(.+?)\}/g, function(match, group1) {
    // TODO extend to :parameters (not port)?
    if (!server.variables) {
      server.variables = {};
    }
...
function fixInfo(openapi, options, reject) {
  ...
  if (
    typeof openapi.info.title === "undefined" ||
    openapi.info.title === null
  ) {
    if (options.patch) {
      openapi.info.title = "";
    } else {
  ...
  if (
    typeof openapi.info.version === "undefined" ||
  ...
  if (typeof openapi.info.logo !== "undefined") {
    if (options.patch) {
      ...
      return reject(
        new S2OError("(Patchable) info should not have logo property")
      );
  }
  if (typeof openapi.info.termsOfService !== "undefined") {
    if (openapi.info.termsOfService === null) {
      if (options.patch) {
        ...
        return reject(
          new S2OError("(Patchable) info.termsOfService cannot be null")
        );
      }
    }
    if (url.URL && options.whatwg) {
      try {
        ...
        if (options.patch) {
          ...
          return reject(
            new S2OError("(Patchable) info.termsOfService must be a URL")
          );
      }
    }
  }
  ...
  return maybe(
    ...
    new Promise(function(resolve, reject) {
      ...
      openapi.openapi =
        ...
        options.targetVersion.startsWith("3.")
      ...
      if (swagger["x-ms-parameterized-host"]) {
        ...
        let server = {};
        server.url =
          xMsPHost.hostTemplate + (swagger.basePath ? swagger.basePath : "");
        server.variables = {};
